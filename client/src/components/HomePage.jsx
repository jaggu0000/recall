...
const SEED_PERSONS = [
  {
    ...
    memories: [
      {
        id: 1,
        memory: "Loves afternoon tea by the garden window.",
        priority: 1,
        tags: ["Home", "Food"],
      },
      {
        id: 2,
        memory: "Used to sing Edelweiss every Sunday morning.",
        priority: 2,
        tags: ["Music", "Family"],
      },
    ],
  ...
  },
  {
    ...
    memories: [
      {
        id: 3,
        memory: "Was a carpenter for 40 years, loves the smell of cedar.",
        priority: 1,
        tags: ["Work", "Childhood"],
      },
    ],
  ...
  },
  {
    ...
    memories: [
      {
        id: 4,
        memory: "Met her husband at a jazz concert in 1972.",
        priority: 1,
        tags: ["Music", "Travel"],
      },
      {
        id: 5,
        memory: "Favourite dish is her mother's lamb stew.",
        priority: 2,
        tags: ["Food", "Family"],
      },
      {
        id: 6,
        memory: "Retired schoolteacher, still loves crosswords.",
        priority: 3,
        tags: ["Hobby"],
      },
    ],
      ...
      memories: [
        {
          ...
          tags: ["Family"],
          embedding: [],
          _id: "69c88dfc7ea83a2a3a6c1243",
          time_created: "2026-03-29T02:27:08.195Z",
          time_updated: "2026-03-29T02:27:08.195Z",
        },
      ],
      embedding: [],
      _id: "69c88dfc7ea83a2a3a6c1242",
    },
    textData: null,
    addedBy: "caregiver",
    createdAt: "2026-03-29T02:27:08.195Z",
    updatedAt: "2026-03-29T02:27:08.195Z",
    __v: 0,
  },
];
...
export default function HomePage({ userId }) {
  ...
  const filtered = persons?.filter(
    (p) =>
      p?.personData?.name.toLowerCase().includes(search.toLowerCase()) ||
      p?.personData?.relation.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    const fetchPersons = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/memory-vault/${userId}`,
          {
            method: "GET",
        ...
        // assuming backend returns array of persons
    ...
    <div className="min-h-screen bg-stone-950 flex font-serif">
      ...
      <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-stone-900 border-r border-stone-800 min-h-screen px-6 py-8">
        ...
        <div className="mt-8 space-y-3">
          ...
          <StatPill
            ...
            value={persons.reduce(
              (s, p) => s + p?.personData?.memories.length,
              0,
            )}
          ...
          <StatPill
            label="High Priority"
            value={
              persons.filter((p) => p?.personData?.person_priority <= 2).length
            }
            color="text-rose-400"
          />
        </div>
      ...
      >
        ...
        <div className="px-6 py-6">
          <PersonForm onClose={() => setDrawerOpen(false)} userId={userId} />
        </div>
      </div>
...
function PersonTile({ person, index, onClick }) {
  const ringColor =
    PRIORITY_RING[person.person_priority - 1] || "ring-stone-300";
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-white rounded-t-3xl border-b border-stone-100 px-6 py-5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-2xl ${avColor} flex items-center justify-center text-lg font-bold`}
              >
                {person.images?.[0]?.url ? (
                  <img
                    src={person.images[0].url}
          ...
          <div className="px-6 py-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Detail
                label="Memories"
                value={`${person.memories.length} recorded`}
              />
            </div>
                      ...
                      {mem.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {mem.tags.map((t) => (
                            <span
                              key={t}
                              className="text-[10px] px-2 py-0.5 bg-white border border-amber-200 rounded-full text-amber-700 font-medium"
                            >
                              {t}
                            </span>
                          ))}
...
// ─────────────────────────────────────────────────────────────────────────────
function PersonForm({ onClose, userId }) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/memory-vault/`,
        {
          ...
          body: JSON.stringify({
            patientId: userId,
            type: "person",
            addedBy: "caregiver", // or dynamic

            personData: {
              name: newPerson.name,
              dateOfBirth: newPerson.dateOfBirth,
              relation: newPerson.relation,
              priority: newPerson.relation,
              memories: newPerson.memories, // initially empty
            },
          }),
        },
      );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600">
              Form has been reset — ready for the next person.
            </p>
          </div>
        </div>
      </div>
      ...
      <FormSection num="01" title="Person Information">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <FormLabel>
              Priority{" "}
              <span className="text-stone-400 font-normal text-[10px] normal-case">
                (1 = highest)
              </span>
            </FormLabel>
            <input
            ...
            {imagePreviews.map((img, i) => (
              <div key={i} className="relative w-16 text-center">
                <img
                  src={img.url}
                  alt={img.name}
                  className="w-16 h-16 object-cover rounded-xl border-2 border-amber-100"
                />
                <button
                  type="button"
      ...
      <FormSection
        ...
        action={
          <button
            type="button"
            onClick={addMemory}
            className="flex items-center gap-1.5 bg-stone-800 text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-stone-700 active:scale-95 transition"
          >
      ...
      >
        {memories.length === 0 && (
          <div className="border-2 border-dashed border-emerald-200 bg-emerald-50 rounded-xl py-8 text-center">
            ...
            <p className="text-xs text-stone-400 mt-0.5">
              Click <strong>+ Add Memory</strong> to begin.
            </p>
          </div>
        )}
        <div className="flex flex-col gap-4">
          {memories.map((mem, idx) => (
            <div
              key={mem.id}
              className="border border-amber-100 rounded-xl bg-amber-50 p-4 flex flex-col gap-3"
            >
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-amber-500 tracking-widest uppercase font-mono">
                  Memory #{idx + 1}
                </span>
                <button
                  type="button"
              ...
              <div className="flex flex-col gap-2">
                  ...
                  {SUGGESTED_TAGS.filter((t) => !mem.tags.includes(t)).map(
                    (t) => (
                      <button
                        ...
                        className="text-[11px] px-2.5 py-0.5 rounded-full border border-emerald-300 bg-emerald-50 text-emerald-700 font-semibold hover:bg-emerald-500 hover:text-white transition"
                      >
                        + {t}
                      </button>
                    ),
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-amber-100 border border-amber-300 text-stone-700 text-xs font-semibold px-3 rounded-lg hover:bg-amber-200 transition"
                  >
                    Add
                  </button>
                </div>
                {mem.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {mem.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center gap-1 bg-stone-800 text-white text-xs font-semibold px-2.5 py-0.5 rounded-full"
                      >
                        {tag}
                        <button
                          type="button"
                          onClick={() => removeTag(mem.id, tag)}
                          className="text-white/60 hover:text-white font-bold leading-none"
                        >
                          ×
                        </button>
                      </span>
      ...
      <div className="flex items-center justify-between bg-white rounded-2xl border border-amber-100 px-5 py-4">
        <p className="text-xs text-stone-400 italic">
          <span className="font-bold text-stone-700">{memories.length}</span>{" "}
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="text-sm text-stone-500 font-semibold px-4 py-2.5 rounded-xl hover:bg-stone-100 transition"
          >
            Cancel
          </button>
          <button
